...
export function DashboardHeader({ name, subtitle, subtitleDesktopExtra }: DashboardHeaderProps) {
  ...
  return (
    <div className="mb-4 flex flex-col gap-3 rounded-2xl border border-l-[6px] border-primary bg-card p-3.5 shadow-xs sm:mb-6 sm:flex-row sm:items-center sm:justify-between sm:gap-4 sm:p-5">
      <div className="flex min-w-0 items-center gap-2.5 sm:gap-3.5">
        ...
        {user?.employeeId ? (
          <Link to="/ess/profile" aria-label="View my profile" className="shrink-0 rounded-full active:scale-95">
            {avatar}
          </Link>
        ) : (
          avatar
