...
export function ResetPasswordPage() {
  ...
  async function handleSubmit(event: FormEvent) {
    ...
    try {
      ...
      setError(
        extractError(err, 'Could not reset your password. The link may be invalid or expired.')
      );
    ...
    return (
      <div className="flex min-h-screen items-center justify-center bg-page px-4">
        <div className="w-full max-w-sm rounded-xl border border-border bg-card p-8 text-center shadow-sm">
          ...
          <h1 className="mt-4 text-base font-semibold text-ink">Invalid reset link</h1>
          <p className="mt-1.5 text-sm text-ink-muted">
            This link is missing its reset token. Please use the exact link from your email, or
            request a new one.
          </p>
        </div>
      </div>
