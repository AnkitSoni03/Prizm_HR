...
async function listRosterGroups({ companyId, brandId, scopedBrandIds, limit, offset }) {
...
async function getRosterGroupForWrite({ companyId, id, scopedBrandIds }) {
  const rosterGroup = await db.RosterGroup.findOne({ where: { id, companyId } });
  if (!rosterGroup) throw new HttpError(404, 'Roster Group not found');
  assertBrandWriteScope({ scopedBrandIds, recordBrandId: rosterGroup.brandId });
  return rosterGroup;
}

async function createRosterGroup({
  companyId,
  brandId,
  scopedBrandIds,
  name,
...
async function updateRosterGroup({ companyId, id, updates, updatedBy, scopedBrandIds }) {
  const rosterGroup = await getRosterGroupForWrite({ companyId, id, scopedBrandIds });
  const { name, description, brandId } = updates;
...
async function deleteRosterGroup({ companyId, id, scopedBrandIds }) {
  const rosterGroup = await getRosterGroupForWrite({ companyId, id, scopedBrandIds });
...
// (wrong company, or — for a brand-scoped caller — outside their brand) are
// collected and skipped rather than aborting the whole batch, same shape as
// shiftRoster.service.js::bulkCreateShiftRoster.
async function bulkAssignRosterGroup({ companyId, id, employeeIds, scopedBrandIds }) {
  await getRosterGroupForWrite({ companyId, id, scopedBrandIds });

  const results = [];
  for (const employeeId of employeeIds) {
    const employeeWhere = { id: employeeId, companyId };
    if (scopedBrandIds) employeeWhere.brandId = { [db.Sequelize.Op.in]: scopedBrandIds };
    const employee = await db.Employee.findOne({ where: employeeWhere });
    if (!employee) {
